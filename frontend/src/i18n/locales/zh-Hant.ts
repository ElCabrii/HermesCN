/**
 * zh-Hant (繁體中文) locale catalog.
 *
 * The legacy `static/i18n.js` had NO separate zh-Hant block: its `zh` block
 * was itself Traditional Chinese (`_lang: 'zh-Hant'`, `_speech: 'zh-TW'`).
 * So this catalog mirrors the ported `zh.ts` content verbatim so both the
 * `zh` and `zh-Hant` codes resolve to the same Traditional Chinese strings
 * (matching what the legacy frontend actually shipped). Keys the legacy
 * block lacked fall back to `en` via the i18n fallback chain.
 */
